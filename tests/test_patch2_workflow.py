...
class Patch2WorkflowTests(unittest.TestCase):
    ...
    def test_outer_camera_stop_failure_is_nonzero_and_persisted(self):
        ...
        self.assertEqual(metadata["runner_variant"], "camera")
        self.assertFalse(metadata["include_final_iti"])
        self.assertEqual(metadata["final_iti_policy"], "skipped_before_poststim_gray")
        self.assertFalse(metadata["final_iti_played"])
        self.assertEqual(metadata["final_iti_duration_sec"], 0.0)
        self.assertEqual(metadata["final_planned_iti_duration_sec"], 0.7)
